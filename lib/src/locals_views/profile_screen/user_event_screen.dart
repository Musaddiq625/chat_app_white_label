...
class UserEventScreen extends StatefulWidget {
  ...
  UserEventScreen({super.key, required this.userId});
...
class _UserEventScreenState extends State<UserEventScreen> {
  ...
  void initState() {
    ...
    //   }
    // }
    viewUserScreenCubit.fetchAllEventGroupData(widget.userId, "event");
        ...
        children: [
          ...
          Expanded(
            child: GridView.builder(
              ...
              itemCount: viewUserScreenCubit.userEventGroupWrapper.data?.events?.length,
              ...
              itemBuilder: (BuildContext context, int index) {
                ...
                // print("viewUserScreenCubit.userEventGroupWrapper.data?.groups?[index] ${viewUserScreenCubit.userEventGroupWrapper.data?.events?[index]}");
                var event = viewUserScreenCubit.userEventGroupWrapper.data?.events?[index]; //eventList[index];
                ...
                return GestureDetector(
                    ...
                    EventCard(
                      title: event?.title ?? "",
                      totalCount: (event?.acceptedCount ?? 0) as int,
                      ...
                      e.image ??
                          ...
                          .toList(),
                      startTime: (event?.venue ?? []).first.startDatetime,
                      endTime: (event?.venue ?? []).first.endDatetime,
                      radius2: 20,
