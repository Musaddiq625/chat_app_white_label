class HttpConstants {
  static const String base = 'https://locals.weuno.co';

  // static const String login = '/login/email';
  // static const String signUp = '/signup';
  static const String sendOtp = '/users/send-otp';
  static const String verifyOtp = '/users/verify-otp';
  static const String forgetPassword = '/forgot-password';
  static const String updateFcm = '/users/update-fcm';
  static const String friendRequest = '/users/friendRequest';
  static const String users = '/users';
  static const String eventGroups = '/event/user/events-groups';
  static const String allEventGroups = '/event/events-groups';
  static const String notifications = '/notifications';
  static const String moreAbout = '/more_abouts';
  static const String interests = '/interests';
  static const String event = '/event';
  static const String categories = '/categories';
  static const String eventReport = '/event/report';
  static const String eventYouGoingTo = '/event/going';
  static const String userLogout = '/users/logout';
  static const String myEvent = '/event/myEvent';
  static const String earnings = '/users/earnings';
  static const String userBankDetail = '/user_bankdetails';
  static const String updateUserBankDetail = '/user_bankdetails/';
  static const String withDrawAmount = '/transactions/amount-withdraw/';
  static const String myFriendList = '/users/friendlist';
  static const String groupsData = '/event/groups';
  static const String eventVisibility = '/event/isVisibility';
  static const String shareGroup = '/event/groups-invites';
  static const String eventDisLike = '/event/dislike';
  static const String eventRequest = '/event/requestUpdate';
  static const String getTicket = '/get_tickets';
}
