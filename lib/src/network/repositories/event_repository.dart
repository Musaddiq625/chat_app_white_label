...
class EventRepository {
  ...
  // static Future<EventResponseWrapper> sendEventRequestQueryAndStatus(String eventId,String eventRequestId,String requestStatus,Query queryReply) async {
  //   final response =
  //   await getIt<DioClientNetwork>().putRequest("${HttpConstants.event}/$eventId",{
  //     "event_request":
  //   });
  ...
  static Future<TicketModelWrapper> buyTicketRequest(TicketModel ticketModel) async {
    final response =
    await getIt<DioClientNetwork>().putRequest("${HttpConstants.getTicket}/",
        ticketModel.toJson()
    );
